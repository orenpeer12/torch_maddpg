...
if not sys.platform.startswith('win'):
    ...
    ray.init()
    @ray.remote(num_cpus=1, num_gpus=1/9)
    def ray_run(run_num):
        return run_main(run_num)
...
if __name__ == '__main__':
    runs = []
    print(Arglist().num_runs)
    for run_num in range(Arglist().num_runs):
        if USE_RAY:
            runs.append(ray_run.remote(run_num))
        else:
            run_main(run_num)
    ray.get(runs)
print(time.time() - tt)